...
function fnLogout() {
	...
	getElementById("txtLoginCustId").value = ""; // 로그인 아이디
...
function fnLogin() {
	...
	var sCustId = elmCustId.value; // 아이디
	...
	xmlHttpRequest(TRAN_CUST_ID + "getCustomer.do", "GET_USER", custparam, completeLogin);
}

// 로그인 시도 카운트 증가
// param : 조회 결과 정보
function fnUpdateTryCnt(param) {
	xmlHttpRequest(TRAN_CUST_ID + "updateTryCnt.do", "UPD_TRY_CNT", param, completeLogin);
}

// 로그인 성공으로 시도 카운트 0으로 초기화
// param : 조회 결과 정보
function fnUpdateTryCntInit(param) {
	xmlHttpRequest(TRAN_CUST_ID + "updateTryCntInit.do", "INIT_TRY_CNT", param, completeLogin);
}
...
function completeLogin(callbackId, status, rqstParam, rspsResult) {
	...
	if(callbackId == "GET_USER") { // 로그인 fnLogin
		var custId = rqstParam.custId;
		var pwd = rqstParam.pwd;
		if(isNull(rspsResult)) { // 아이디로 사용자 정보 찾을 수 없음
			if(confirm(custId + " 아이디는 등록되지 않은 아이디 입니다.\n" + custId + " 아이디로 회원가입 하시겠습니까?")) {
				fnJoin(custId); // 회원 가입 div 창 띄우기 fnJoin
			}
		...
		if(pwd == rspsResult.pwd) { // 비밀번호 일치
			getElementById("sectionLogin").style.display = "none"; // 로그인 section 보여주기
			
			getElementById("txtCustId").value = rspsResult.custId; // 메인 창 아이디 셋팅
			getElementById("txtCustName").value = rspsResult.custName; // 메인 창 이름 셋팅
